...
pub fn lindstedt_poincare(
    ...
    for _step in 0..num_steps {
        ...
        compute_forces(molecule, list_potentials.clone(), false);


        // Compute the normal modes and frequencies
        let (normal_modes, mut frequencies) = get_normal_modes_and_frequencies(molecule, list_potentials.clone()).unwrap();
        if frequencies.iter().any(|&x| x.is_nan()) {
            // You may choose to handle it in some specific way, like replacing NaNs with some default value:
            for i in 0..frequencies.len() {
                if frequencies[i].is_nan() {
                    frequencies[i] = 0.0;  // or any other value you consider appropriate
                }
            }
        }
        ...
        molecule.coordinates = molecule.clone().coordinates + higher_order_solution.clone();
        ...
        println!("At -------");
        println!("The energy is : {:?}", molecule.energy);
        //println!("coords : {:?}", molecule.coordinates);
        //println!("velocs : {:?}", molecule.velocities);
    ...
    fn update_verlet_with_perturbed_frequency(
        ...
        perturbed_frequencies = perturbed_frequencies.into_shape((3 * molecule.num_atoms, )).unwrap();
        let square_frequencies = perturbed_frequencies
                .mapv(|f| f * f)
                .into_shape((molecule.num_atoms, 3)).unwrap();
        ...
        /* TODO!: make a new perturbed molecule struct to solve this */
        // Update positions using Verlet algorithm with perturbed frequency in the normal mode space
        let updated_coords = reshaped_coords.clone() + molecule.clone().velocities * time_step
        ...
        molecule.coordinates = normal_modes.t().dot(&updated_coords.into_shape((molecule.num_atoms * 3, )).unwrap()).into_shape((molecule.num_atoms, 3)).unwrap();
        ...
        molecule.velocities = old_velocities + 0.5 * force_mol.clone().forces / molecule.mass_n_by_3() * time_step;
    }
...
pub fn compute_higher_order_corrections(
    ...
    for j in 0..frequencies.len() {
        ...
        for order in 1..=max_order {
            ...
            let correction_adder = (nth_derivative + 1e-13)/ (factorial(order as u64) as f64)* (epsilon * frequency + 1e-13).powi(order as i32);
