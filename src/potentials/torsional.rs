...
pub fn torsional_energy(molecule: &mut Molecule, energy_constant: f64, save_to_molecule: bool) -> f64 {
    // Calculate the current torsion angles for each torsion
    let current_torsion_angles: Vec<_> = molecule.equilibrium_torsion.par_iter()
        .map(|&(i, j, k, l, _)| {
    ...
    // Wrap mutable shared data with Arc<Mutex<T>>
    let molecule_energy = Arc::new(Mutex::new(0.0));
    let molecule_torsion_current = Arc::new(Mutex::new(molecule.torsion_current.clone()));


    // Compute the torsional energy
    molecule.equilibrium_torsion.par_iter().enumerate()
        .for_each(|(index, &(i, j, k, l, equilibrium_torsion_angle))| {
            ...
            if save_to_molecule && molecule_torsion_current.lock().unwrap().len() > 0 {
                molecule_torsion_current.lock().unwrap()[index].4 = current_torsion_angles[index];
            }

            *molecule_energy.lock().unwrap() += torsional_energy;
        });

        let final_energy = *molecule_energy.lock().unwrap();

        if save_to_molecule {
            molecule.energy += *molecule_energy.lock().unwrap();
            molecule.torsion_current = Arc::try_unwrap(molecule_torsion_current).unwrap().into_inner().unwrap();
        }
...
pub fn torsional_energy_serial(molecule: &mut Molecule, energy_constant: f64, 
